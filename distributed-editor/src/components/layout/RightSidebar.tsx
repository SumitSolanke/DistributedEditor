...
import { Send, Hash, Lock, Link2, Search } from "lucide-react"
...
export default function RightSidebar({ isOpen }: { isOpen: boolean }) {
  ...
  const messages = useMemo(() => {
    ...
    return baseMessages.filter(
      (m) => m.text.toLowerCase().includes(s) || m.from.toLowerCase().includes(s)
    )
  }, [baseMessages, q])
  ...
  }, [isOpen, tab, selectedDmUserId, unreadPublic, unreadDm, markPublicRead, markDmRead])
  ...
  const handleSend = () => {
    ...
    const lineRef =
      attachLine && activeFileId ? { fileId: activeFileId, line: cursor.line } : undefined
      ...
      <div className="w-full h-full flex flex-col bg-[#1f1f1f] text-white overflow-hidden">
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {lineRef && (
                          <button
                            onClick={() => revealInEditor(lineRef.fileId, lineRef.line)}
        ...
        <div className="border-t border-gray-700 p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            >
              {activeFileId ? `Attach line (Ln ${cursor.line})` : "Open file to attach line"}
            </button>
          </div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder={tab === "public" ? "Message #public..." : "Message (DM)..."}
            />
